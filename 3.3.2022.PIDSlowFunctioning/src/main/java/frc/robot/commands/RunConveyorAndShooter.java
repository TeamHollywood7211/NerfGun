...
public class RunConveyorAndShooter extends CommandBase {
  ...
  @Override
  public void execute() {
    while(BreakBeams.ammo>0 && RobotContainer.shootHighTrigger.get()){ // Got bullets? Button mashed?
      Shooter.SetShootersHighPID();
      while(Shooter.shooterLeftEncoder.getVelocity() <= targetShooterVelocityHigh && Shooter.shooterRightEncoder.getVelocity() <= targetShooterVelocityHigh){}//waits to do everything else until the motors reach a targeted rpm
